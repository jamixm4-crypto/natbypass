...
old_manifest_path = Path(r"e:\qwen\fnat\mirror\releases\v1.9.226-beta8.json")
new_manifest_path = Path(r"e:\qwen\fnat\mirror\releases\v1.9.226-beta9.json")
...
for key, asset in manifest.get("assets", {}).items():
    ...
    if "url" in asset:
        asset["url"] = asset["url"].replace("v1.9.226-beta8", "v1.9.226-beta9")
    if "sig_url" in asset:
        asset["sig_url"] = asset["sig_url"].replace("v1.9.226-beta8", "v1.9.226-beta9")
    if "mirrors" in asset:
        asset["mirrors"] = [m.replace("v1.9.226-beta8", "v1.9.226-beta9") for m in asset["mirrors"]]
