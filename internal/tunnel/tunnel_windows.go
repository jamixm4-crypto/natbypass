...
func CreateAdapter(adapterName, virtualIP string) (*Device, error) {
	...
	var psInit string
	if ifIndex > 0 {
		psInit = fmt.Sprintf(`Get-NetIPAddress | Where-Object { $_.IPAddress -eq "%s" -and $_.InterfaceIndex -ne %d } | Remove-NetIPAddress -Confirm:$false -ErrorAction SilentlyContinue; Get-NetRoute | Where-Object { ($_.DestinationPrefix -eq "%s.0/24" -or $_.DestinationPrefix -eq "100.64.200.0/24") -and $_.InterfaceIndex -ne %d } | Remove-NetRoute -Confirm:$false -ErrorAction SilentlyContinue; netsh interface ipv4 set address name=%d source=static address="%s" mask=255.255.255.0; Set-NetIPInterface -InterfaceIndex %d -DadTransmits 0 -InterfaceMetric 1 -RouterDiscovery Disabled -ErrorAction SilentlyContinue; Set-NetConnectionProfile -InterfaceIndex %d -NetworkCategory Private -ErrorAction SilentlyContinue; New-NetRoute -InterfaceIndex %d -DestinationPrefix "%s.0/24" -NextHop 0.0.0.0 -RouteMetric 1 -ErrorAction SilentlyContinue; New-NetRoute -InterfaceIndex %d -DestinationPrefix "100.64.200.0/24" -NextHop 0.0.0.0 -RouteMetric 1 -ErrorAction SilentlyContinue; netsh advfirewall firewall add rule name="NatBypass ICMPv4 In" dir=in action=allow protocol=ICMPv4 enable=yes; Enable-NetFirewallRule -DisplayGroup "Core Networking Diagnostics" -ErrorAction SilentlyContinue; Enable-NetFirewallRule -DisplayGroup "File and Printer Sharing" -ErrorAction SilentlyContinue`, cleanVIP, ifIndex, prefix, ifIndex, ifIndex, cleanVIP, ifIndex, ifIndex, ifIndex, prefix, ifIndex)
	} else {
