...
func runEngine(ctx context.Context, cfg *config.Config, enableTray bool) error {
	...
	if tunErr != nil {
		...
		lastReactivePunchCLI := make(map[string]time.Time)
		lastReactiveDialCLI := make(map[string]time.Time)
		go func() {
			for {
				select {
				case <-engineCtx.Done():
				...
				case pkt, ok := <-pktCh:
					...
					if found && p != nil {
						...
						if !isForceTCP && !sentDirect && !sentTCP && puncher != nil {
							if now.Sub(lastReactivePunchCLI[p.DeviceID]) > 3*time.Second {
						...
						if !isForceUDP && !sentTCP && !sentDirect && tcpDirectMgr != nil && !tcpDirectMgr.HasConn(p.DeviceID) {
							if now.Sub(lastReactiveDialCLI[p.DeviceID]) > 5*time.Second {
								lastReactiveDialCLI[p.DeviceID] = now
								defaultTCPPort := 8443
								netKey := ""
