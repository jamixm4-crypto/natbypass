...
// getPeerTCPTarget resolves the best TCP address for a peer (TCPAddr > PublicIP:port > LocalAddr:port).
func getPeerTCPTarget(p *peer.Peer, defaultPort int) string {
	if p == nil {
		return ""
	}
	if p.TCPAddr != "" {
...
// getSignalTCPTarget resolves the best TCP address from an incoming signaling beacon.
func getSignalTCPTarget(p *signaling.Payload, defaultPort int) string {
	if p == nil {
		return ""
	}
	if p.TCPAddr != "" {
	...
	if tunErr != nil {
		...
		onInboundPacket := func(payload []byte, directAddr *net.UDPAddr, isTCP bool, isRelay bool) {
			...
			// Recalculate IPv4 header checksum to guarantee Windows kernel Wintun accepts packet unconditionally
				...
				// If destination is NOT my Virtual IP, forward directly to destination peer over active TCP/UDP connection
				if inDstIP != cleanMyVIP && cleanMyVIP != "" && registry != nil {
					if destPeer, ok := registry.GetByVirtualIP(inDstIP); ok && destPeer != nil && destPeer.DeviceID != deviceID {
						// IPv4 TTL decrement to strictly prevent routing loops
						if len(payload) >= 20 && payload[0]>>4 == 4 {
							ttl := payload[8]
							if ttl <= 1 {
								log.Debug().Str("src", inSrcIP).Str("dst", inDstIP).Msg("TTL expired in mesh relay, dropping packet")
								return
							}
							payload[8] = ttl - 1
							ihl := int(payload[0]&0x0F) * 4
						...
						// 1. Forward over active direct TCP ShadowTLS to destination peer
						if tcpDirectMgr != nil && tcpDirectMgr.HasConn(destPeer.DeviceID) {
							if err := tcpDirectMgr.SendPacket(destPeer.DeviceID, payload); err == nil {
								log.Debug().Str("from", inSrcIP).Str("to", inDstIP).Str("via_tcp", destPeer.DeviceID).Msg("🔀 Mesh Relay: Forwarded packet via TCP ShadowTLS")
								return
							}
						}
						// 2. Forward over Direct UDP to destination peer
						if puncher != nil && destPeer.DirectP2P && destPeer.ActiveEndpoint != "" {
							if err := puncher.SendDataPacketWithPadding(destPeer.ActiveEndpoint, payload, 0, 0); err == nil {
								log.Debug().Str("from", inSrcIP).Str("to", inDstIP).Str("via_udp", destPeer.DeviceID).Msg("🔀 Mesh Relay: Forwarded packet via Direct UDP")
								return
							}
						}
					}
		...
		lastReactivePunchEngine := make(map[string]time.Time)
		lastReactiveDialEngine := make(map[string]time.Time)
		go func() {
			for {
				select {
				case <-engineCtx.Done():
			...
			for {
				...
				case pkt, ok := <-pktCh:
					...
					if found && p != nil {
						...
						// Dual-send to LocalAddr for LAN clients ONLY if send to targetEP failed or targetEP empty
						if !isForceTCP && !sentTCP && !sentDirect && p.LocalAddr != "" && p.LocalAddr != targetEP && puncher != nil {
							if err := puncher.SendDataPacketWithPadding(p.LocalAddr, pkt, pmin, pmax); err == nil {
								sentDirect = true
							}
						}
						bilateralOK := p.IsBilateralP2P(constants.BilateralDemotionThreshold - 10*time.Second)

						// Also try STUNAddr ONLY IF direct send hasn't succeeded
						if !isForceTCP && !sentTCP && !sentDirect && (!p.DirectP2P || !bilateralOK) && puncher != nil && p.STUNAddr != "" && p.STUNAddr != targetEP {
							if p.Transport == "quic" {
								_ = puncher.SendQUICPacket(p.STUNAddr, pkt)
							} else {
								if err := puncher.SendDataPacketWithPadding(p.STUNAddr, pkt, pmin, pmax); err == nil {
									sentDirect = true
								}
							}
						}
						// 1c. Reactive instant hole punching if direct P2P is not yet confirmed AND direct send failed (throttled to 3s)
						now := time.Now()
						if !isForceTCP && !sentDirect && !sentTCP && puncher != nil {
							if now.Sub(lastReactivePunchEngine[p.DeviceID]) > 3*time.Second {
								lastReactivePunchEngine[p.DeviceID] = now
								if targetEP != "" {
						...
						if !isForceUDP && !sentTCP && !sentDirect && tcpDirectMgr != nil && !tcpDirectMgr.HasConn(p.DeviceID) {
							if now.Sub(lastReactiveDialEngine[p.DeviceID]) > 5*time.Second {
								lastReactiveDialEngine[p.DeviceID] = now
								defaultTCPPort := 8443
								netKey := ""
						...
						// Triggered if direct TCP not connected AND direct bilateral UDP failed:
						if needsRelay {
							if wssClient != nil && wssClient.IsConnected() {
								...
								go func(targetID string, rawPkt []byte) {
									...
									_ = sigMgr.PublishTunnelData(targetID, dataToSend)
								}(pID, pktCopy)
							}
						}
					}
			...
			for {
				...
				case <-kaTicker.C:
					...
					for _, p := range registry.List() {
						...
						// Evaluated on every tick, NOT blocked by UDP probe backoff or ghost DirectP2P status!
