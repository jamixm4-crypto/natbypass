...
##### 🔧 Диагностика и решение проблем на Windows Server
Если в браузере отображается ошибка подключения:
1. **Проверьте прослушивание порта:**
   ```cmd
   netstat -an | findstr 8080
   ```
   Должна отображаться строка со статусом `LISTENING` на `127.0.0.1:8080` или `0.0.0.0:8080`.
2. **Проверьте правило брандмауэра:**
   ```cmd
   netsh advfirewall firewall show rule name="NatBypass WebUI"
   ```
3. **Используйте пункт в трее:** Нажмите правой кнопкой на иконку в системном трее и выберите **«🩺 Диагностика WebUI»** — консоль выведет полный отчёт о сокете, брандмауэре и ответе HTTP эндпоинта `/healthz`.
